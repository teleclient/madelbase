...
'with the phrase "Text to show in message" instead of this help message.

Created by [Daniil Gentili](mention:@danogentili) (@daniilgentili) using the '.
'[MadelineProto PHP MTProto client](daniil.it/MadelineProto).';
            ...
            case 'updateNewMessage':
                ...
                try {
                    if (isset($update['update']['message']['message']) &&
                        preg_match('|/start|', $update['update']['message']['message']))
                    {
            ...
            case 'updateNewChannelMessage':
                ...
                try {
                    if (preg_match('|/start|', $update['update']['message']['message'])) {
                        $MadelineProto->messages->sendMessage([
                            'peer'            => $update['update']['message']['to_id'],
                            'message'         => $start,
                            'reply_to_msg_id' => $update['update']['message']['id'],
                            'parse_mode'      => 'markdown',
                            'reply_markup'    => $reply_markup
            ...
            case 'updateBotInlineQuery':
                try {
                    ...
                    if ($update['update']['query'] === '') {
                        $MadelineProto->messages->setInlineBotResults([
                        ...
                        if (empty($rows)) {
                            ...
                            $toset['results'] = [[
                                ...
                                'id'           => (string) random_int(0, pow(2, 31) - 1),
                                'type'         => 'article',
                                'title'        => $text,
                                'description'  => 'Your keyboard',
                                'send_message' => [
